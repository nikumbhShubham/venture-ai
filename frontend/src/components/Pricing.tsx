...
// // price_1S5BVk4t9oIYFFE9k5AeUdBM product id monthly
// // price_1S5Bd14t9oIYFFE9EUTBFnpm  product id yearly
// // IMPORTANT: Replace these with your actual Price IDs from your Stripe Dashboard in Test Mode
// const STRIPE_PRO_MONTHLY_PRICE_ID = "price_1S5BVk4t9oIYFFE9k5AeUdBM"; // Your monthly price ID
// const STRIPE_PRO_YEARLY_PRICE_ID = "price_1S5Bd14t9oIYFFE9EUTBFnpm"; // Your yearly price ID
...
//     "monthly"
//   );
//   const [isLoading, setIsLoading] = useState<string | null>(null); // Track loading state for each plan
//   const { token } = useAuthStore();
//   const navigate = useNavigate();

//   const handleSubscription = async (planName: string) => {
//     if (!token) {
//       navigate("/signup"); // If not logged in, send them to sign up
//       return;
//     }
//     // For this example, we only handle the "Pro" plan subscription
//     if (planName !== "Pro") {
//       // You could navigate to the dashboard for the free plan
//       if (planName === "Hobby") navigate("/dashboard");
//       // Or open a mailto link for enterprise
//       if (planName === "Enterprise")
//         window.location.href = "mailto:shubhamnikumbh19@gmail.com";
//       return;
//     }
...
//     <div
//       id="pricing"
//       className="relative w-full text-white font-sans py-24 sm:py-32"
//     >
...
//         >
//           Start for free and scale as you grow. All plans include access to our
//           powerful AI creative engine.
//         </motion.p>
...
//             }
//             className={`w-14 h-8 flex items-center bg-slate-700 rounded-full p-1 cursor-pointer transition-colors ${
...
//               <Link
//                 to="/signup"
//                 className={`mt-10 block w-full py-3 px-6 text-center font-semibold rounded-lg transition-all duration-300 group ${
//                   plan.isFeatured
...
const Pricing = () => {
  ...
  const handleSubscription = async (planName: string) => {
    ...
    if (planName !== "Pro") {
      ...
      // Or open a mailto link for enterprise
      if (planName === "Enterprise")
        window.location.href = "mailto:sales@ventureai.com";
      return;
    }
        ...
        >
          {pricingPlans.map((plan) => (
            ...
            >
              ...
              <ul className="mt-8 space-y-4 text-left h-48">
                {plan.features.map((feature) => (
                  <li key={feature} className="flex items-center gap-3">
                    <CheckCircle2 className="w-5 h-5 text-teal-400" />
                    <span className="text-slate-300">{feature}</span>
              ...
              >
                <div className="flex items-center justify-center gap-2">
                  {isLoading === plan.name ? (
                    <>
                      <Loader className="w-5 h-5 animate-spin" /> Redirecting...
                    </>
                  ) : (
                    <>
                      <span>{plan.cta}</span>
