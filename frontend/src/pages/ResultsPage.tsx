...
import { User, Briefcase, Heart, AlertTriangle } from "lucide-react";
  ...
  <div className="grid grid-cols-2 md:grid-cols-4 gap-4 mt-2">
    {Object.entries(palette).map(([name, hex]) => (
      <div key={name} className="text-center">
        <div
          className="w-full h-20 rounded-lg border-2 border-slate-700"
...
const PersonaDisplay = ({ persona }: { persona: any }) => {
  ...
  try {
    ...
    return <p className="text-slate-400">{persona}</p>;
  }
  ...
  return (
    <div className="space-y-4">
      <div className="flex items-center gap-3">
        ...
        <div>
          <h4 className="text-xl font-bold text-white">{parsed.name}</h4>
      ...
      <div className="space-y-3">
        <div>
          <h5 className="font-semibold text-purple-300 mb-1">Lifestyle</h5>
          <p className="text-slate-400">{parsed.lifestyle}</p>
        </div>
        <div>
          <h5 className="font-semibold text-purple-300 mb-1">Needs</h5>
          <p className="text-slate-400">{parsed.needs}</p>
        </div>
        <div>
          <h5 className="font-semibold text-purple-300 mb-1">Pain Points</h5>
          <p className="text-slate-400">{parsed.pain_points}</p>
        </div>
      </div>
    </div>
  );
          ...
          <div className="grid grid-cols-1 lg:grid-cols-3 gap-12">
            ...
            >
              ...
              <div className="bg-slate-800/50 p-6 rounded-2xl border border-white/10 shadow-lg">
                ...
                <a
                  ...
                  className={`mt-4 w-full flex items-center justify-center gap-2 bg-purple-600 text-white font-bold py-2 px-4 rounded-lg transition-colors ${
                    kit.logoConcept && !kit.logoConcept.includes("failed")
                      ? "hover:bg-purple-700"
            ...
            <div className="lg:col-span-2 space-y-8">
              ...
              <BrandSection
                ...
                delay={0.1}
              >
                <PersonaDisplay persona={kit.targetPersona} />
              </BrandSection>

              {/* Brand Story */}
              <BrandSection icon={BookOpen} title="Brand Story" delay={0.2}>
                <blockquote className="italic text-lg text-slate-300 border-l-4 border-purple-500 pl-4">
                  {kit.brandStory}
                </blockquote>
              </BrandSection>
              ...
              <BrandSection icon={Palette} title="Visual Identity" delay={0.3}>
                {kit.colorPalette && (
                  <ColorPaletteDisplay palette={kit.colorPalette} />
                )}
                {kit.fontPairing && (
                  <div className="mt-6 pt-6 border-t border-slate-700">
                    <FontPairingDisplay fonts={kit.fontPairing} />
                  </div>
                )}
              </BrandSection>
              ...
              >
                ...
                {kit.marketingChannels && (
                  <MarketingList items={kit.marketingChannels} />
                )}
              </BrandSection>
              ...
              <BrandSection icon={Lightbulb} title="Content Ideas" delay={0.5}>
                ...
                {kit.contentPillars && (
                  <MarketingList items={kit.contentPillars} />
                )}
